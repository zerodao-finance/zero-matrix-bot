...
const { getChainName, getLink } = require("./helper");
require("olm");
const os = require("os");
...
const formatDestination = (s) =>
  s.substr(0, 8) === "0x626331"
    ? Buffer.from(s.substr(2), "hex").toString("utf8")
    : ethers.utils.base58.encode(s);
...
const MatrixBot = (exports.MatrixBot = class MatrixBot {
  ...
  constructor({ homeserverUrl, redis }) {
    ...
    const storage = new SimpleFsStorageProvider(
      path.join(this.constructor.storageDirectory, "simple.json")
    );
    const cryptoStorage = new RustSdkCryptoStorageProvider(
      this.constructor.storageDirectory
    );
    let client;
    let queueLength;
    Object.assign(this, {
    ...
    try {
      if (newLength > this.queueLength) {
        ...
        for (let i = difference - 1; i >= 0; i--) {
          ...
          if (chain == "Optimism") {
            assetName = await this.getAssetOp(assetAddr);
          }
          if (requestType == "burn") {
            ...
            Owner: ${owner}<br>
            Owner: <a href="${link}${owner}"/>${owner}</a>
            Destination: <a href="https://mempool.space/address/${formatDestination(destination)}"/>${formatDestination(destination)}</a>
            </samp>`;
          }
          if (requestType == "transfer") {
            ...
            Destination: <a href="${link}${to}"/>${to}</a>
            </samp>`;
          }
          console.log(message);
  ...
  async sendGeneral(msg) {
    await this.client.sendHtmlText(process.env.MATRIX_BOT_ROOMNAME, msg);
  }
  async sendAll(msg) {
  ...
  async getAssetOp(address) {
    switch (ethers.utils.getAddress(address)) {
      case "0x7F5c764cBc14f9669B88837ca1490cCa17c31607":
        return "USDC";
      case "0x85f6583762Bc76d775eAB9A7456db344f12409F7":
        return "renBTC";
      case "0x68f180fcCe6836688e9084f035309E29Bf0A2095":
        return "WBTC";
      default:
        return "OP";
    }
