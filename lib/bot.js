...
const MatrixBot = (exports.MatrixBot = class MatrixBot {
  ...
  async listen() {
    ...
    try {
      ...
      const request = await this.redis.lindex("/zero/request", 50);
      ...
      if (requestType == "burn") {
        ...
        message = `<div><h3>Burn</h3>
            <code>
            <ul>
            <li>Chain: ${chain}</li>
            <li>Amount: ${formAmount} ${assetName}</li>
            <li>Owner: ${owner}</li>
      ...
      if (requestType == "transfer") {
        ...
        message = `<div><h3>Mint</h3>
            <code>
            <ul>
            <li>Chain: ${chain}</li>
            <li>Amount: ${formAmount} BTC => ${assetName}</li>
            <li>Destination: ${destination}</li>
            </ul>
            </code></div>`;
        //   message =
  ...
  async sendGeneral(msg) {
    await this.client.sendHtmlText(
      "!utjhpieYKaZJxTIYWQ:matrix.zerodao.gg",
      msg
    );
  }
  async sendAll(msg) {
